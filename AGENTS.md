# AI Repository Index — HRM 391

## Purpose

This public repository is the course archive for HRM 391. Class Scribe adds lecture notes after a recording is transcribed and passes automated quality checks. Use this file as the entry point before answering questions from the repository.
...
Enumerate them dynamically with `rg --files notes`, keep only those dated patterns, and sort by path. Do not rely on a hand-maintained lecture list: new files arrive automatically. HRM 391 normally has two recordings per class week. A date may be one day earlier than the scheduled class day because the recording app has produced that convention.

`notes/week-01/week-01-organization-environment-fit.md` is a study primer, not a transcript. Use it only as supplemental material and label it as such.
